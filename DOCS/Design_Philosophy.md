...
As the number of motors increases, the drone generally becomes more stable because thrust gets distributed across multiple motors. More motors also allow the drone to carry additional payloads such as larger batteries, cameras, sensors, or extra electronics. Some larger configurations like hexacopters and octacopters can even remain partially controllable after a motor failure, which is useful in professional applications.
...
Because of this, larger configurations are usually used only when the application actually requires them.
